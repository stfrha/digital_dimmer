*** readme.txt ***
Spartan-3E Sample Pack LED Counter Demo
...
Pack board.  The design shows the center LED counting every second. Every 10 seconds
another LED is lit until all 6 LEDs are on thus indicating 60 seconds have passed.
Further, this design demonstrates the MultiBoot functionality.
...
- Open the project 'LED_counter.ise' in Project Navigator. Note:  You can not have two
  instances of Project Navigator open at the same time.
- Highlight 'LED_counter' in the 'Sources in Project' pane.  If you want to make
  changes or would like to see the HDL code then double click on 
  'LED_counter.v' in 'Sources in Project' pane. This will open the Verilog file 
...
- In the 'Processes for Source' pane double click on 'Generate Programming File'
	...
	'LED_counter.bit' in the project directory.
...
- To download the design to the FPGA using the Intel StrataFlash refer to the
	...
	PROMGEN command:
	promgen -w -p bin -c FF -o LED_counter_bpi_up.bin -u 0 LED_counter.bit

	Note:  In this case LED_counter_bpi_up.bin is the output binary file but can
		 be renamed to the name of the user's choice.
------------------------------------------------------------------------------------
----	FILE STRUCTURE for LED Counter distribution					  ----	
------------------------------------------------------------------------------------

/ {root}
readme.txt 			--> This file
LED_counter.ise 		--> ISE project file
LED_counter.v		--> Verilog file for LED Counter
LED_counter.ucf		--> Pin location constraints for Spartan-3E Sample Pack board
